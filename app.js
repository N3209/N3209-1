...
function renderLaw(lawEl) {
  ...
  function pushIndex(anchor, text) {
    ...
    if (t) index.push({ anchor, label: anchorLabel(anchor), text: t, norm: normalize(t) });
  }
  ...
  function renderItemNode(el, depth, anchor) {
    ...
    const titleTag = tag === 'Item' ? 'ItemTitle' : tag + 'Title';
    const sentTag = tag === 'Item' ? 'ItemSentence' : tag + 'Sentence';
...
function lawByName(name) {
  if (!name) return null;
  const exact = state.laws.find(l => l.lawTitle === name || l.abbrev === name);
 ...
 */
function bestAnchor(index, scope, art, par, item, strict) {
  ...
  if (!par && item) {
    ...
    if (hits.length === 1) return hits[0].anchor;
    if (hits.length > 1 && strict) return null;
    if (hits.length > 1) return hits[0].anchor;
  }
  // strict のときは、指定された項・号が無ければ諦める。
  // 参照リンクは「第99号」と書いてあるのに条へ飛ばす、という黙った読み替えをしない。
  if (strict && item) return null;
  if (par && has(`${scope}/${art}/${par}`)) return `${scope}/${art}/${par}`;
  if (strict && par) return null;
...
function resolveRelative(a, pane) {
  ...
  return bestAnchor(pane.index, scope, targetArt, targetPar, wantItem, true);
}
  ...
  for (const a of $$('a.ref', pane.el)) {
    ...
    if (a.dataset.rel) {
      ...
      if (law && idx) {
        ...
        target = bestAnchor(idx, 'M', art, par, item, true);
        lawId = law.lawId;
      }
    } else {
      const [scope, art, par, item] = a.dataset.ref.split('/');
      target = bestAnchor(pane.index, scope, art, par, item, true);
      if (scope !== 'M') {
        // 附則の中の裸の「第七百三十八条」は、その附則の条とも本則の条とも読める。
        // 両方にあるなら決め手が無いのでリンクしない。片方だけならそれを指す。
        const inMain = bestAnchor(pane.index, 'M', art, par, item, true);
...
function renderMarkList() {
  ...
  for (const n of state.notes.values()) {
    if (n.color) byColor.set(n.color, (byColor.get(n.color) || 0) + 1);
  ...
  for (const c of used) {
    ...
    li.onclick = () => showMarkResults(n => n.color === c.key, `${c.label}のマーク`);
    ml.appendChild(li);
  }
  ...
  for (const [tag, n] of [...byTag].sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0], 'ja'))) {
    ...
    li.onclick = () => showMarkResults(x => (x.tags || []).includes(tag), `タグ「${tag}」`);
    tl.appendChild(li);
  }
}
...
function renderNotePane() {
  ...
  tagInput.onkeydown = async e => {
    ...
    if (!v || note.tags.includes(v)) { tagInput.value = ''; return; }
    note.tags.push(v);
    tagInput.value = '';
    await saveNote(note);
    paintChips();
...
const rangeKeyOf = r => r.lawId + ':' + r.anchor;
...
function showMarkResults(pred, title) {
  ...
  const hits = [...state.notes.values()].filter(pred);
  $('#find-count').textContent = `${title}　${hits.length}件`;
  box.innerHTML = '';
  for (const n of hits) {
    const law = state.laws.find(l => l.lawId === n.lawId);
    const idx = state.indexCache.get(n.lawId);
    const entry = idx && idx.find(e => e.anchor === n.anchor);
    const d = document.createElement('div');
    d.className = 'r';
    d.innerHTML = `<div class="sub">`
      + (n.color ? `<span class="sw" style="display:inline-block;width:10px;height:10px;border-radius:3px;background:var(--mk-${n.color});vertical-align:-1px;margin-right:6px"></span>` : '')
      + `${esc(law ? law.lawTitle : n.lawId)}　${esc(anchorLabel(n.anchor))}</div>`
      + (entry ? `<div class="snippet">${esc(entry.text.slice(0, 110))}</div>` : '')
      + (n.memo ? `<div class="snippet" style="color:var(--fg-dim)">📝 ${esc(n.memo.slice(0, 80))}</div>` : '');
    d.onclick = () => { $('#dlg-find').close(); navigate(n.lawId, n.anchor); };
    box.appendChild(d);
  }
  $('#dlg-find').showModal();
}
...
/** e-Gov から受け取ったままのXML。加工しない。 */
async function exportOriginal(lawId) {
  const rec = await store.getLaw(lawId);
  if (!rec) return;
  download(`${safeName(rec.lawTitle)}.xml`, rec.xml, 'application/xml');
  toast('オリジナルXMLを書き出しました');
}

/**
 * 注釈を埋め込んだXML。
 * 対象要素の先頭に <Note> を1つ挿入するだけにしてある。
 * 妥当なXMLであることを崩さない（壊れたXMLを書き出す実装を見たことがあるため）。
 */
async function exportAnnotated(lawId) {
  const rec = await store.getLaw(lawId);
  if (!rec) return;
  const doc = parseXML(rec.xml);
  const lawEl = doc.querySelector('Law');
  ...
  const mine = [...state.notes.values()].filter(n => n.lawId === lawId);
  let placed = 0, orphan = 0;

  for (const n of mine) {
    const target = elementForAnchor(lawEl, n.anchor);
    if (!target) { orphan++; continue; }
    const note = doc.createElement('Note');
    note.setAttribute('anchor', n.anchor);
    if (n.summary) note.setAttribute('summary', n.summary);
    if (n.color) note.setAttribute('color', n.color);
    if (n.tags && n.tags.length) note.setAttribute('tags', n.tags.join(','));
    if (n.memo) note.setAttribute('memo', n.memo);
    if (n.updatedAt) note.setAttribute('updated', new Date(n.updatedAt).toISOString());
    target.insertBefore(note, target.firstChild);
    placed++;
  }

  for (const r of state.ranges.values()) {
    if (r.lawId !== lawId) continue;
    const target = elementForAnchor(lawEl, r.anchor);
    if (!target) { orphan++; continue; }
    const q = doc.createElement('Phrase');
    q.setAttribute('anchor', r.anchor);
    q.setAttribute('text', r.text);
    q.setAttribute('nth', String(r.nth || 1));
    if (r.color) q.setAttribute('color', r.color);
    if (r.memo) q.setAttribute('memo', r.memo);
    target.insertBefore(q, target.firstChild);
    placed++;
  }

  const out = new XMLSerializer().serializeToString(doc);
  download(`${safeName(rec.lawTitle)}_注釈付き.xml`, out, 'application/xml');
  toast(`注釈 ${placed}件を埋め込みました` + (orphan ? `（位置不明 ${orphan}件は除外）` : ''));
}

function openExportDialog() {
  const cur = P().current;
  $('#export-law').innerHTML = cur
    ? `<p class="note-label">開いている法令：${esc(cur.lawTitle)}</p>`
      + `<div class="row"><button type="button" id="exp-orig" class="mini">オリジナルXML</button>`
      + `<button type="button" id="exp-anno" class="mini">注釈付きXML</button></div>`
    : '<p class="hint">法令を開くと、その法令だけを書き出せます。</p>';

  if (cur) {
    $('#exp-orig').onclick = () => exportOriginal(cur.lawId);
    $('#exp-anno').onclick = () => exportAnnotated(cur.lawId);
  }
...
function bindPaneEvents(pane) {
  pane.el.addEventListener('click', e => {
    ...
    if (!el) return;
    selectAnchor(el.dataset.anchor, true, pane);
  });
